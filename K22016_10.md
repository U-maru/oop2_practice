...
　チーム内で1つのファイルを編集し合うとmainブランチへPushをおこなうタイミングによってはファイルの変更箇所の衝突が起きてしまう。このような問題の解決策としてGitHub Flowの採用が挙げられる。GitHub Flowでの作業は以下の手順で行う。
1.mainブランチを基準とした作業ブランチを作成
2.作業ブランチ上でファイルの追加・変更を行う
3.自分のブランチ上で動作確認をおこなった行った後にmainブランチへPull Requestを作成
4.Pull Requestのレビューを他のメンバーへ依頼
5.レビューを行ったメンバーは問題の有無をコメントに書き、問題が無さそうであればmainブランチにmergeを行う
<参考文献>(https://gist.github.com/Gab-km/3705015)